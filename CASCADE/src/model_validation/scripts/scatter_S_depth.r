...
a <- read.table( paste( input_dir , "/PSAL_total_data.txt", sep="" ) , header=FALSE , sep=";")
date_start = min(as.Date(a[,4]))
date_fin   = max(as.Date(a[,4]))

pdf(file = paste( output_dir , "/scatter_meas-sal_" , gr , "_", date_start , "_to_" , date_fin , ".pdf", sep="" ) )
...
axis(3, at=c(26:40) , labels=c(26:40) , las=0  )
title( main=paste("Salinity scatter plot" , " Data:", gr ," Period:", se_label) , line=2.4 , cex.main=1.1 , outer=FALSE )
