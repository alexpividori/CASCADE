...
#                   different SHYFEM simulations for salinity
# 
# EXTERNAL CALLS:   none.
#
...
for (i in 1:info_dim[1]){  # ralize n taylor diagrams one for each depth level
   ...
   pdf(file = paste( output_dir ,"/taylor_multi-diagram_SHY_S_" , gr , "_" , x_names[i]  , ".pdf", sep="" ) )
...
taylor.diagram( ref=a[,7]  ,  model=a[,8] ,
              ...
              pch=19,
              pos.cor=TRUE,
              xlab=paste("Standard Deviation [PSU]", sep=""),
              ylab=paste("Standard Deviation [PSU]", sep=""),
