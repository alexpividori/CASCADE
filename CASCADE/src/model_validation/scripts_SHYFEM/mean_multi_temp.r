#******************************************************************************
#
# DESCRIPTION:      Instead of a boxplot, this scrimpt places 
#                   on an xy Cartesian diagram a changing color
#                   marker for different SHYFEM simulation
#                   
# 
# EXTERNAL CALLS:   none.
#
...
se_label=se
 ...
 pdf(file = paste( output_dir ,"/mean_multi-diagram_SHY_T_" , gr , ".pdf", sep="" ) )
...
if ( mean(a[,8] - a[,7] ) < y_min ){
y_min <- mean(a[,8] - a[,7] )
}

if ( mean(a[,8] - a[,7] ) > y_max ){
y_max=mean(a[,8] - a[,7])
}
...
#=============================================================================
	...
	if ( j==1 && i==1 ){
        par(oma=c(0, 0, 0, 7))
	plot( depth_inf[i] + (depth_sup[i] - depth_inf[i])/2  , mean(a[,8] - a[,7] )  ,
              col="black",
              bg=j,
              pch=21,  
              lwd=0.2,
              ...
              xaxt='n',
              xlab="Depth[m]",
              ylab=paste("Thind-Tmeas [\U00B0" , "C]", sep=""),
              main=paste("SHYFEM model (Thind-Tmeas) Mean value \n Stations:", gr , " Period:", se_label )
              )
	} else {
	points(depth_inf[i] + (depth_sup[i] - depth_inf[i])/2 , mean(a[,8] - a[,7] ), pch = 21, lwd=0.2 , bg=j, col="black" )
	}
#=============================================================================
