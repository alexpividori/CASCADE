...
info_file  <- read.csv2( paste( "./selected_depths_" , gr ,"_", se, "/selected_T_files_bydepth.txt" , sep="") , header=FALSE )
...
for (i in 1:info_dim[1]){
   ...
   x_names[i] = paste( info_file[i,2] , "÷" , info_file[i,3] , sep="" )
    ...
    sink(paste( output_dir , "/p-values_temp_zero.comp_", gr , "_", se ,".txt", sep="" ) , append=TRUE ) 
      print(paste("t-test TEMP ",gr , se, ": null hipotesys (compatibility with 0), 
