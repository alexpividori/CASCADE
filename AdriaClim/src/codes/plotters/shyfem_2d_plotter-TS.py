...
#                   of temperature (left y-axis) and salinity (right y-axis),
#                   starting from the information retrieved in an
...
#                      where
#
#                         input_file-ia = i-th temperature time series input
#                                         file (see the following point)
#                         input_file-ib = i-th salinity time series input file
...
def plot(x1, x2, y1, y2, location, input_file_name):
    ...
    x_label = "Time"
    y_label_left = "Temperature (vertically averaged) [°C]"
    axes_fontsize = 18
    ...
    y_label_right = "Salinity (vertically averaged) [PSU]"
    ...
    fig_name = "TS." + os.path.basename(input_file_name).split(".")[1] + "." + os.path.basename(input_file_name).split(".")[2]  + ".png"
...
for line in init_file:
    ...
    date_time_temp = data_collect(input_file_name1)[0]
    date_time_temp = dt_format(date_time_temp)           # Function calling: format
                                                         # the date and time by building
                                                         # "datetime" objects
    temp = data_collect(input_file_name1)[1]
    date_time_salt = data_collect(input_file_name2)[0]
    date_time_salt = dt_format(date_time_salt)           # Function calling: format
                                                         # the date and time by building
                                                         # "datetime" objects
    salt = data_collect(input_file_name2)[1]

    # Function calling: time series plot
    plot(date_time_temp, date_time_salt, temp, salt, loc, input_file_name1)
