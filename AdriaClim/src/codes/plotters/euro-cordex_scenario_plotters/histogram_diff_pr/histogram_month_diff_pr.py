...
plt.title("Differences for daily precipitation distribution (rcp"+RCP+") on "+months_list[ int(month) - 1 ]+" from "+str(year_in)+" to "+str(year_fin)+"\n respect 2010/2020 decade at point: "+point_name+"\n" , fontsize=28, fontdict={'family':'serif'} )
plt.xlabel( x_axis_label , fontsize=25 )
plt.hlines( 0.0 , -1. , 5*len(data_bin_pos)+1. , linestyle="--" )
plt.ylabel( "Delta Relative frequency ($"+months_list[int(month) - 1 ]+"_{"+year_in+"/"+year_fin+"}-"+months_list[int(month) - 1 ]+"_{2010/2020}$)", fontsize=25)
...
plt.savefig( output_arc_dir+"/"+point_name+"/histogram_diff_"+field_name+"_"+point_name+"_rcp"+RCP+"_"+str(month).rjust(2, '0')+"_"+year_in+"_"+year_fin+".png" )
