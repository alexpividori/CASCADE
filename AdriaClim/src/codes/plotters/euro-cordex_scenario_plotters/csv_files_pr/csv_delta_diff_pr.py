...
if (not os.path.exists('AdriaClim_GoT_allscenario_delta-relative-frequency_pr.csv') ):
	with open( 'AdriaClim_GoT_allscenario_delta-relative-frequency_pr.csv', 'w') as f_out:
		f_out.write("# Delta daily precipitation relative frequencies respect to 2010-2020 decade\n")
		f_out.write("# Point; Month; Decade; RCP; bin-range [mm/day]; delta [1]\n")

with open( 'AdriaClim_GoT_allscenario_delta-relative-frequency_pr.csv', 'a') as f_out:
	for i in range(len(count_diff)):
		f_out.write( point_name + ";" + format(int(month),'02') + ";" + year_in + "-" + \
