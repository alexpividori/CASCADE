...
echo -e "\tI'm executing $dir_name/$file_name"
...
echo  "# Statistical Estimator= Delta Median"                                   >> $file_out
...
if [[ -f $file_t ]]; then
	cdo -outputf,%2.4g,1  -selindexbox,$(( ind_x + 1 )),$(( ind_x + 1 )),$(( ind_y + 1 )),$(( ind_y + 1 ))  -selname,tos_med  $file_t > $tmp_temp  
else
	cat $missing_file  > $tmp_temp
fi

if [[ -f $file_s ]]; then
	cdo -outputf,%2.4g,1  -selindexbox,$(( ind_x + 1 )),$(( ind_x + 1 )),$(( ind_y + 1 )),$(( ind_y + 1 ))  -selname,sos_med  $file_s > $tmp_sal  
else
...
if [[ -f $file_z &&  $file_z != */AdriaClim-IPSL-CM5A-MR_LMD-LMDZ4NEMOMED8/*_zos_*rcp85*  &&  $file_z != */EC-EARTH_CLMcom-GUF-CCLM5-0-9-NEMOMED12-3-6/*_zos_*rcp85*  ]]; then
	cdo -outputf,%2.4g,1  -selindexbox,$(( ind_x + 1 )),$(( ind_x + 1 )),$(( ind_y + 1 )),$(( ind_y + 1 ))  -selname,zos_med  $file_z > $tmp_zos   
else
