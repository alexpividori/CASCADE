...
CDO_RESET_HISTORY=1    # this attribute is necessary to reset the history attribute
export CDO_RESET_HISTORY

#********************************************************************

for model_dir in "3_EC-EARTH_CCLM4-8-17" "4_EC-EARTH_RACMO22E"
do

dir_name="/lustre/arpa/AdriaClim/data/EURO-CORDEX/FVG/${model_dir}/postproc/monthly"
init_tmp="initialization_file.txt"

ls $dir_name  > $init_tmp || { echo "Something has gone wrong during the file list creation."; exit 1 ; }
...
file_name_out="new_$file_name"
	...
	cdo  --no_history -setattribute,product="Monthly mean of model daily outputs"                                                                 \
                          ...
                          $dir_name/$file_name   $dir_name/$file_name_out                                                                             \
